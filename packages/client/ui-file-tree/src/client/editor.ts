...
import type { FileEditorState } from './store.ts'

/** The bound action set needed by the editor helper. */
export type EditorActions = {
  markOpened: (path: string) => void
  setEditor: (editor: FileEditorState | null) => void
}
 ...
 * @param actions - the store's bound actions.
 * @param readFile - host read face.
 * @param path - absolute file path to open.
 */
export function openEditor(actions: EditorActions, readFile: ReadFile, path: string): void {
